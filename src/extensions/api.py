...
class Api(commands.Cog):
    ...
    async def lmgtfy(self, ctx, *query):
        await ctx.send(f"http://lmgtfy.com/?q={urlencode(query)}")
    ...
    async def lmddgtfy(self, ctx, *query):
        await ctx.send(f"http://lmddgtfy.net/?q={urlencode(query)}")
    ...
    async def urban(self, ctx, * query : str):
        url = f"http://api.urbandictionary.com/v0/define?term={urlencode(query)}"
        result, error = await get_json(url)
        if error:
            await ctx.send(error)
            return
    ...
    async def hoogle(self, ctx, * query : str):
        ...
        url = f"https://hoogle.haskell.org?mode=json&hoogle={urlencode(query)}&start=1&count=1"
        result, error = await get_json(url)
        if error:
            await ctx.send(error)
            return
        embed = discord.Embed(
            ...
            url=f"https://hoogle.haskell.org/?hoogle={urlencode(query)}",
            color=self.bot.embed_color)
        embed.set_thumbnail(
